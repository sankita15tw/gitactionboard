...
public class AuthenticationController {
  ...
  public AuthenticationController(@Qualifier("basicAuthenticationManager") AuthenticationManager authenticationManager) {
  ...
  public ResponseEntity<Object> login(@RequestBody AuthRequest authRequest, HttpServletResponse response){
    ...
    try {
      ...
      if (authentication.isAuthenticated()){
        ...
        response.addCookie(createCookie(ACCESS_TOKEN, createAccessToken(username, password), ONE_DAY));
  ...
  private String createAccessToken(String username, String password){
    final byte[] encodedAuth = Base64.encodeBase64(String.format("%s:%s", username, password).getBytes(UTF_8) );
